...
func (mb *client) ReadCoils(address, quantity uint16) (results []byte, err error) {
	...
	if count != length {
		err = NewModbusProtocolError("modbus: response data size '%v' does not match count '%v'", length, count)
		return
	}
...
func (mb *client) WriteSingleCoil(address, value uint16) (results []byte, err error) {
	...
	if len(response.Data) != 4 {
		err = NewModbusProtocolError("modbus: response data size '%v' does not match expected '%v'", len(response.Data), 4)
		return
	}
	respValue := binary.BigEndian.Uint16(response.Data)
	if address != respValue {
		err = NewModbusProtocolError("modbus: response address '%v' does not match request '%v'", respValue, address)
		return
	}
	results = response.Data[2:]
	respValue = binary.BigEndian.Uint16(results)
	if value != respValue {
		err = NewModbusProtocolError("modbus: response value '%v' does not match request '%v'", respValue, value)
		return
	}
	return
}
...
func (mb *client) WriteMultipleCoils(address, quantity uint16, value []byte) (results []byte, err error) {
	...
	if quantity != respValue {
		err = NewModbusProtocolError("modbus: response quantity '%v' does not match request '%v'", respValue, quantity)
		return
	}
	return
}
...
func (mb *client) MaskWriteRegister(address, andMask, orMask uint16) (results []byte, err error) {
	...
	if len(response.Data) != 6 {
		err = NewModbusProtocolError("modbus: response data size '%v' does not match expected '%v'", len(response.Data), 6)
		return
	}
	respValue := binary.BigEndian.Uint16(response.Data)
	if address != respValue {
	...
	if andMask != respValue {
		err = NewModbusProtocolError("modbus: response AND-mask '%v' does not match request '%v'", respValue, andMask)
		return
	}
	respValue = binary.BigEndian.Uint16(response.Data[4:])
	if orMask != respValue {
		err = NewModbusProtocolError("modbus: response OR-mask '%v' does not match request '%v'", respValue, orMask)
		return
	}
	results = response.Data[2:]
	return
...
func (mb *client) ReadWriteMultipleRegisters(readAddress, readQuantity, writeAddress, writeQuantity uint16, value []byte) (results []byte, err error) {
	...
	if count != (len(response.Data) - 1) {
		err = NewModbusProtocolError("modbus: response data size '%v' does not match count '%v'", len(response.Data)-1, count)
		return
	}
	results = response.Data[1:]
	return
...
func (mb *client) ReadFIFOQueue(address uint16) (results []byte, err error) {
	...
	if len(response.Data) < 4 {
		err = NewModbusProtocolError("modbus: response data size '%v' is less than expected '%v'", len(response.Data), 4)
		return
	}
	...
	if count > 31 {
		err = NewModbusProtocolError("modbus: fifo count '%v' is greater than expected '%v'", count, 31)
		return
	}
...
func (mb *client) send(request *ProtocolDataUnit) (response *ProtocolDataUnit, err error) {
	...
	if response.Data == nil || len(response.Data) == 0 {
		// Empty response
		err = NewModbusProtocolError("modbus: response data is empty")
		return
	}
	return
}
